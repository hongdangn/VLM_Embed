...
def main():
    ...
    for subset in tqdm(data_args.subset_name, desc="Iterate datasets to calculate scores"):
        ...
        for row in tqdm(eval_data, desc=f"calculate score for {subset}"):
            if model_args.model_backbone == COLPALI:
                ...
                pos_rowid = tgtkey_to_rowid[pos_tkey]
                ranked_ids = np.argsort(-scores_np)  # descending order
                rank = np.where(ranked_ids == pos_rowid)[0][0] + 1  # 1-indexed rank
                mrr_sum += 1.0 / rank
                
                pred_id = ranked_ids[0]
                ...
                if rank <= 3:
                    n_correct_p3 += 1
        score_path = os.path.join(data_args.encode_output_path, f"{subset}_score.json")
        mrr = mrr_sum / len(eval_data)
        print(f"\033[91m{subset} P@1 accuracy: {n_correct_p1/len(eval_data)}\033[0m")
        print(f"\033[91m{subset} P@5 accuracy: {n_correct_p3/len(eval_data)}\033[0m")
        print(f"\033[91m{subset} MRR: {mrr}\033[0m")
        
        score_dict = {
            "P@1": n_correct_p1/len(eval_data), 
            "P@3": n_correct_p3/len(eval_data), 
            "MRR": mrr,
            "num_correct_p1": n_correct_p1, 
            "num_correct_p3": n_correct_p3,
    ...
    if score_sets:
        avg_p1 = sum(s["P@1"] for s in score_sets) / len(score_sets)
        avg_p3 = sum(s["P@3"] for s in score_sets) / len(score_sets)
        avg_mrr = sum(s["MRR"] for s in score_sets) / len(score_sets)
        print(f"\033[92m========== Average Metrics Across All Subsets ==========\033[0m")
        print(f"Avg P@1: {avg_p1:.4f}")
        print(f"Avg P@3: {avg_p3:.4f}")
