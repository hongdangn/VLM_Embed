...
def main():
    ...
    for p in model_trainer.model.parameters():
        ...
        p.data = p.data.to(torch.bfloat16) # Ép kiểu bfloat16 toàn bộ cho nhẹ

    # 2. CHỈ mở cờ cho layer cuối cùng mà bạn muốn lấy đạo hàm

    # target_layers = model_trainer.target_layers # Sửa lại đường dẫn layer cho đúng với kiến trúc của bạn
    # for target_layer in target_layers:
    #     print_rank(f"Unfreezing layer: {target_layer}")
    #     for p in target_layer.parameters():
    #         p.requires_grad = True
    #         num_trainable_vision += p.numel()
    for p in model_trainer.target_params:
        p.requires_grad = True
        num_trainable_vision += p.numel()
