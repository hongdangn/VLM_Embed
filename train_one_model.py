...
def finetune(
    ...
    model_engine, optimizer, _, lr_scheduler = deepspeed.initialize(
        model=model_trainer,
        optimizer=optimizer,
        model_parameters=model_trainer.model.parameters(),
    ...
    for epoch in range(training_args.num_train_epochs):
        ...
        if getattr(model_engine, "global_rank", 0) == 0:
            ...
            if training_args.save_strategy == "epoch":
                ...
                unwarpped_model = model_engine.module.model
                
                if hasattr(unwarpped_model, 'peft_config'):
                    unwarpped_model.peft_config.save_pretrained(ckpt_dir)
                    unwarpped_model.encoder.save_pretrained(ckpt_dir)
    ...
    if getattr(model_engine, "global_rank", 0) == 0 and training_args.save_strategy == "epoch":
        ...
        unwarpped_model = model_engine.module.model
        unwarpped_model.encoder.save_pretrained(final_ckpt_dir)
        if hasattr(unwarpped_model, 'peft_config'):
